...
| 서비스 | 용도 |
|---|---|
| `_DataService` | `WeaponData`, `WeaponLevelData` 로드 |
...
### 5-1. 초기화 (OnBeginPlay)
`WeaponData` 로드 → 무기별 `required_exp`, `start_level`, `max_level` 캐싱.
...
`WeaponSwapComponent` 슬롯 데이터에도 반영 (`WeaponSwapComponent.CaptureRuntimeToSlot`).

```
ApplyLevelUpServer(weaponId, newLevel):
  dmgMultiplier = WeaponLevelData[weaponId]["level_" .. newLevel]
  FireSystemComponent.BaseWeaponAttack = baseAtk * dmgMultiplier
