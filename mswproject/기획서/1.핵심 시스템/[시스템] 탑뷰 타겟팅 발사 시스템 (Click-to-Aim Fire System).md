...
0. [시작] :  플레이어의 모델 (msw 엔진의 프리팹) 아래에 무기 모델 (msw 엔진의 프리팹)
   을 두어 플레이어는 이동만 하고 공격 및 회전은 무기가 실시한다.
   
1. **[조준]** 플레이어가 마우스 커서를 목표 지점에 둔다. (이때 무기의 총구 위치는 손잡이 부분을 기준으로 회전하며 마우스의 커서 위치를 바라본다 
    이 때 캐릭터는 마우스가 화면 중앙 기준 좌측에 위치 시 좌측을 바라보고 우측에 위치 시 좌우 반전을 통해 우측을 바라봄 )
...
모든 수치는 데이터 시트에 명세 되어있음 
PlayerData
WeaponData, WeaponLevelData, SummonData
